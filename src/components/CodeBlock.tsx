      ...
      <pre className="min-h-0 overflow-auto py-1 font-mono text-[11px] leading-[1.6]" tabIndex={0} aria-label={block.filename ?? `${block.language} code`}>
        <code>
          {lines.map((line, i) => {
            ...
            return (
              <span key={n} className={cn("flex items-start", hl && "bg-highlight-wash")} aria-current={hl ? "true" : undefined}>
                <span className="w-8 shrink-0 select-none pr-2 text-right text-muted-foreground/60" aria-hidden="true">
                  {n}
                </span>
                <span className={cn("shrink-0 pr-2", hl ? "text-accent" : "text-transparent")} aria-hidden="true">
                  {hl ? "▶" : "·"}
                </span>
                <span className="whitespace-pre-wrap pr-3 [overflow-wrap:anywhere]">{line || " "}</span>
              </span>
            );
          })}
        </code>
